...
:: 결과 CSV 파일 저장
set "csvFile=%resultDir%\W-79-%computerName%.csv"
echo 분류,코드,위험도,진단 항목,진단 결과,현황,대응방안 > "%csvFile%"
echo 보안관리,W-79,상,파일 및 디렉터리 보호,!result!,!status!,파일 및 디렉터리 보호 >> "%csvFile%"

:: 결과 요약 파일 생성
type "%resultDir%\W-79-*" > "%resultDir%\security_audit_summary.txt"
