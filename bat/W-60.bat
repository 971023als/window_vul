...
IF NOT ERRORLEVEL 1 (
    ...
    echo 최대 이벤트 로그 크기가 10,240Kb 이상으로 설정되어 있고, 90일 이상 이벤트 로그를 유지하는 설정이 적용되어 있는 경우 취약 >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    echo 조치 방안 >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    echo 최대 이벤트 로그 크기를 10,240Kb 이상으로 설정하고, 90일 이상 이벤트 로그를 유지하도록 설정을 적용해야 합니다. >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    ...
    echo 작업 시작 >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    echo 최대 이벤트 로그 크기가 10,240Kb 이상으로 설정되어 있고, 90일 이상 이벤트 로그를 유지하는 설정이 적용되어 있는 경우 안전 >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    echo 조치 방안 >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    echo 최대 이벤트 로그 크기를 10,240Kb 이상으로 설정하고, 90일 이상 이벤트 로그를 유지하도록 설정을 적용해야 합니다. >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    type C:\Window_%COMPUTERNAME%_raw\W-60_Application.txt >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
    type C:\Window_%COMPUTERNAME%_raw\W-60_Security.txt >> C:\Window_%COMPUTERNAME%_result\W-Window-%COMPUTERNAME%-result.txt
