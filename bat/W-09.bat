...
for /f "tokens=2 delims==" %%a in ('findstr /C:"PasswordComplexity =" "%rawPath%\Local_Security_Policy.txt"') do (
    ...
    if "!passwordComplexity!"=="1" (
        set "현황=패스워드 복잡성 정책이 적절히 설정되어 있습니다."
    ) else (
        set "진단결과=취약"
        set "현황=패스워드 복잡성 정책이 적절히 설정되지 않았습니다."
    )
)

:: 진단 결과 CSV 파일로 저장
echo 분류,코드,위험도,진단항목,진단결과,현황,대응방안 > "%resultPath%\W-09.csv"
echo 계정관리,W-09,상,패스워드 복잡성 설정,!진단결과!,!현황!,패스워드 복잡성 설정 >> "%resultPath%\W-09.csv"
