...
:: 진단 결과 CSV 파일로 저장
echo 분류,코드,위험도,진단항목,진단결과,현황,대응방안 > "%resultPath%\W-30.csv"
if "!encryptionMethod!"=="ReversibleEncryption" (
    echo 계정 관리,W-30,높음,비밀번호 저장을 위한 복호화 가능한 암호화 사용,취약,복호화 가능한 암호화 방법 사용 중,비밀번호 저장을 위한 복호화 불가능한 암호화 사용 권장 >> "%resultPath%\W-30.csv"
) else (
    echo 계정 관리,W-30,높음,비밀번호 저장을 위한 복호화 가능한 암호화 사용,양호,복호화 불가능한 암호화 방법 사용 중,추가 조치 필요 없음 >> "%resultPath%\W-30.csv"
)
