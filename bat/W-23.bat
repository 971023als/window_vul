...
if exist "!iisConfigFile!" (
    ...
    for /f "delims=" %%a in ('type "%rawPath%\iis_setting.txt" ^| findstr /I "directoryBrowse enabled"') do (
        set "dirBrowseEnabled=%%a"
        if "!dirBrowseEnabled!"=="<directoryBrowse enabled=\"true\"" (
            set "directoryStatus=Enabled"
        ) else (
            set "directoryStatus=Disabled"
        )
    )
)

:: 진단 결과 CSV 파일로 저장
echo 분류,코드,위험도,진단항목,진단결과,현황,대응방안 > "%resultPath%\W-23.csv"
if "!directoryStatus!"=="Enabled" (
    echo 서비스관리,W-23,상,IIS 디렉토리 리스팅 제거,취약,디렉토리 브라우징 활성화,IIS 디렉토리 리스팅 제거 필요 >> "%resultPath%\W-23.csv"
) else (
    echo 서비스관리,W-23,상,IIS 디렉토리 리스팅 제거,양호,디렉토리 브라우징 비활성화,추가 조치 필요 없음 >> "%resultPath%\W-23.csv"
)
